 ...
 * ----------------------------------------------------------------------------
 */
#define _USE_MATH_DEFINES  // Necessario su alcuni sistemi per definire le funzioni matematiche
...
// peak detection                                                  1752
// state machine                                                   1751
int int_sqrt32(int x) {
    return (int)sqrt(x);  // Usa la funzione sqrt dalla libreria math.h e fai il cast a int
}
...
int stepcount_new(int accMagSquared) {
  ...
  //printf("Accel Mag: %d, Filtered Value: %d\n", accMag, v);
 ...
 // printf("Gate Open: %d, Active Sample Count: %d\n", gate_open, active_sample_count);  // Debug: gate aperto o chiuso
  ...
  if (gate_open == true && accFilteredHist[1] > accFilteredHist[0] && accFilteredHist[1] > accFiltered) {
    ...
    //printf("Step detected: %d\n", stepsCounted);  // Debug: passo rilevato
